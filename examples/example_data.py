...
def gen_example_dataset():
    ...
    demo_df_test = pd.DataFrame(data={'demo_Gender': rng.choice(['Male', 'Female'], n_examples_test)})
    ...
    predictor_diffs = np.linspace(-.2, .4, n_predictors)
    ...
    for i, predictor_diff in enumerate(predictor_diffs):
        ...
        X_test[:, i] = y_nodiff_test + 6*rng.randn(n_examples_test) + 6*predictor_diff*male_mask_test
